...
| --------------------- | ------------------------------ | ---------- |
| `GET /vdr/entries`    | Resolve the data using VDR URI | Anyone     |
| `POST /vdr/entries`   | Create a new VDR entry         | Data owner |
| `PUT /vdr/entries`    | Update an existing VDR entry   | Data owner |
...
In this example, we are using a database driver, so the data is resolvable only within the same agent instance.

**Note**

The VDR URL includes the driver information (`drf=database`, `drid=database`) which tells the system
which driver to use when resolving this data later. This is why anyone with access to the agent
can resolve the data - the URL contains all the necessary information.
