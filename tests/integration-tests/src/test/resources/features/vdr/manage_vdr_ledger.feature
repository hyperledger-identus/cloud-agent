@vdr_ledger
Feature: Manage a VDR entry with ledger-backed driver

  # The actual driver (prism-node or neoprism) is selected via the
  # VDR_LEDGER_DRIVER environment variable.  This keeps a single
  # scenario that validates the full CRUD lifecycle regardless of
  # which ledger backend is active.

  Scenario: Create, update, resolve and deactivate a VDR entry
    ...
    When Issuer creates a VDR entry with value 112233 using ledger driver
