...
class VdrServerEndpoints(
  ...
  private val operationStatusEndpoint: ZServerEndpoint[Any, Any] =
    VdrEndpoints.operationStatus
      .zServerLogic { case (rc, operationId) =>
        vdrController
...
object VdrServerEndpoints {
  def all: URIO[VdrController & AuthenticatorWithAuthZ[BaseEntity], List[ZServerEndpoint[Any, Any]]] = {
    for {
