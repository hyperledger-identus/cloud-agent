...
import org.hyperledger.identus.server.config.FeatureFlagConfig
...
case class IssueControllerConfig(
    defaultJwtVCOfferDomain: String,
    httpEndpointServiceName: String,
    httpEndpointPublicUrl: URL,
    issuanceInvitationExpiry: JavaDuration,
    didCommEndpointUrl: URL,
...
class IssueControllerImpl(
            ...
            for {
              ...
              record <- credentialService
                .createJWTIssueCredentialRecord(
                  ...
                  domain = request.domain.getOrElse(config.defaultJwtVCOfferDomain)
                )
            } yield record
          case SDJWT =>
            for {
              ...
              credentialDefinitionId <- {
                ...
                credentialDefinition.resolutionMethod match
                  case ResourceResolutionMethod.did =>
                    val publicEndpointServiceName = config.httpEndpointServiceName
                  ...
                  case ResourceResolutionMethod.http =>
                    val publicEndpointUrl = config.httpEndpointPublicUrl.toExternalForm
                    val httpUrlSuffix =
  ...
  override def createCredentialOfferInvitation(
    ...
    for {
      peerDid <- managedDIDService.createAndStorePeerDID(config.didCommEndpointUrl)
      _ <- checkFeatureFlag(request.credentialFormat.map(CredentialFormat.valueOf).getOrElse(CredentialFormat.JWT))
      offerContext = OfferContext(
        ...
        goal = request.goal,
        expirationDuration = Some(config.issuanceInvitationExpiry)
      )
      result <- createCredentialOfferRecord(request, offerContext)
    } yield result
  }
...
object IssueControllerImpl {
  val layer: URLayer[
    CredentialService & CredentialDefinitionService & ConnectionService & DIDService & ManagedDIDService &
