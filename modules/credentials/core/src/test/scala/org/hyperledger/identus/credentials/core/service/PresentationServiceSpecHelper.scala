...
trait PresentationServiceSpecHelper {
  ...
  protected def requestCredential = org.hyperledger.identus.didcomm.protocol.issuecredential.RequestCredential(
    from = DidId("did:prism:aaa"),
    to = DidId("did:prism:bbb"),
    thid = Some(UUID.randomUUID.toString),
    body = org.hyperledger.identus.didcomm.protocol.issuecredential.RequestCredential
  ...
  extension (svc: PresentationService)
    def createJwtRecord(
        ...
        options: Option[org.hyperledger.identus.credentials.core.model.presentation.Options] = None
