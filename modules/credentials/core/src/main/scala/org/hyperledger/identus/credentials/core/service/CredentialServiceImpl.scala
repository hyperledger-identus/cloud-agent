...
import org.hyperledger.identus.credentials.*
...
import org.hyperledger.identus.credentials.core.model.schema.{CredentialDefinition, CredentialSchema, CredentialSchemaRef}
...
class CredentialServiceImpl(
  ...
  /** @param jwtIssuerDID
    ...
    *   JwtIssuer
    * @see
    *   org.hyperledger.identus.credentials.vc.jwt.Issuer
    */
    ...
    for {
      ...
      w3Credential = W3cCredentialPayload(
        ...
        maybeCredentialSchema = record.schemaUris.map(ids =>
          ids.map(id => org.hyperledger.identus.credentials.vc.jwt.CredentialSchema(id, VC_JSON_SCHEMA_TYPE))
        ),
