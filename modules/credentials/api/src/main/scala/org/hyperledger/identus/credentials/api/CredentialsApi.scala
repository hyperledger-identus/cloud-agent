...
/** Re-exports from credentials.core for the Credentials bounded context API.
  *
  * These type aliases establish the public API surface for the Credentials bounded context. Consumers should depend on
  * credentials-api rather than credentials-core directly. In a future phase, the actual types will be moved here and
