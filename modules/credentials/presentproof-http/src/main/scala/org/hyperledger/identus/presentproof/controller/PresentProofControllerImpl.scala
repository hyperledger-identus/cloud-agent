...
case class PresentProofControllerConfig(
    didCommEndpointUrl: URL,
    presentationInvitationExpiry: JavaDuration,
...
class PresentProofControllerImpl(
    ...
    config: PresentProofControllerConfig
  ...
  override def createOOBRequestPresentationInvitation(request: RequestPresentationInput)(implicit
    ...
    val result: ZIO[WalletAccessContext, ConnectionServiceError | PresentationError, PresentationStatus] = for {
      ...
      record <- createRequestPresentation(
        ...
        proverDID = None,
        request = request,
        expirationDuration = Some(config.presentationInvitationExpiry)
      )
    } yield PresentationStatus.fromDomain(record)
    result
  }
  ...
  def acceptRequestPresentationInvitation(
    ...
    for {
      pairwiseDid <- managedDIDService.createAndStorePeerDID(config.didCommEndpointUrl)
...
object PresentProofControllerImpl {
  val layer: URLayer[
    PresentationService & ConnectionService & ManagedDIDService & PresentProofControllerConfig,
