...
import org.hyperledger.identus.didcomm.model.error.*
import org.hyperledger.identus.didcomm.protocol.routing.*
  ...
  def send(msg: Message): ZIO[DidOps & DidAgent & DIDResolver & HttpClient, SendMessageError, HttpResponse] =
    ZIO.logAnnotate("msgId", msg.id) {
      for {
        ...
        resp <- org.hyperledger.identus.didcomm.HttpClient
