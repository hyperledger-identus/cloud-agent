...
case class DIDCommControllerConfig(connectionsInvitationExpiry: JavaDuration)
...
class DIDCommControllerImpl(
    ...
    config: DIDCommControllerConfig
    ...
    case msg if msg.piuri == ConnectionRequest.`type` =>
      for {
        ...
        record <- connectionService.receiveConnectionRequest(
          connectionRequest,
          Some(config.connectionsInvitationExpiry)
        )
