...
case class CredentialIssuerControllerConfig(httpEndpointPublicUrl: URL)

object CredentialIssuerControllerImpl {
  val layer: URLayer[
    CredentialIssuerControllerConfig & OIDCCredentialIssuerService & OID4VCIIssuerMetadataService,
    CredentialIssuerController
  ] =
    ZLayer.fromZIO(
      for {
        config <- ZIO.service[CredentialIssuerControllerConfig]
      ...
      } yield CredentialIssuerControllerImpl(oidcIssuerService, oidcIssuerMetadataService, config.httpEndpointPublicUrl)
    )
}
