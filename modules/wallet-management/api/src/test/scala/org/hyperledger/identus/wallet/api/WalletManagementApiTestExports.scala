...
/** Test-scope re-exports from agent.walletapi for consumers that need test doubles.
  *
  * These re-exports allow polluxCore tests to depend on walletManagementApi (test->test) instead of cloudAgentWalletAPI
  * directly, completing the reverse dependency break.
  */
