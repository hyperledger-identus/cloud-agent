...
import org.hyperledger.identus.wallet.model.error.{*, given}
import org.hyperledger.identus.wallet.service.handler.{DIDCreateHandler, DIDUpdateHandler, PublicationHandler}
import org.hyperledger.identus.wallet.service.ManagedDIDService.DEFAULT_MASTER_KEY_ID
import org.hyperledger.identus.wallet.storage.{DIDNonSecretStorage, DIDSecretStorage, WalletSecretStorage}
import org.hyperledger.identus.wallet.util.*
import zio.*
...
/** A wrapper around Castor's DIDService providing key-management capability. Analogous to the secretAPI in
  * indy-wallet-sdk.
  */
class ManagedDIDServiceImpl private[wallet] (
    didService: DIDService,
    didOpValidator: DIDOperationValidator,
    private[wallet] val secretStorage: DIDSecretStorage,
    override private[wallet] val nonSecretStorage: DIDNonSecretStorage,
  ...
  private[wallet] def validateInternalKeyRemoval(
