...
import org.hyperledger.identus.did.core.model.did as didDomain
...
object UpdateManagedDIDServiceAction {
  ...
  extension (servicePatch: UpdateManagedDIDServiceAction) {
    def toDomain: Either[String, walletDomain.UpdateServicePatch] =
      for {
        serviceType <- servicePatch.`type`.fold[Either[String, Option[didDomain.ServiceType]]](Right(None))(s =>
          s.toDomain.map(Some(_))
        )
        serviceEndpoint <- servicePatch.serviceEndpoint.fold[Either[String, Option[didDomain.ServiceEndpoint]]](
          Right(None)
