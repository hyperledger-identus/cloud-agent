...
import org.hyperledger.identus.server.jobs.BackgroundJobError.ErrorResponseReceivedFromPeerAgent
          ...
          val issuerPendingToGeneratedFlow = for {
            ...
            ret <- config.featureFlag.ifJWTIsEnabled(
              for {
                ...
                credentialService <- ZIO.service[CredentialService]
                _ <- credentialService
                  .generateJWTCredential(id, config.credentials.statusListRegistry.publicEndpointUrl.toExternalForm)
                  .provideSomeLayer(ZLayer.succeed(walletAccessContext))
            ...
            ret <- config.featureFlag.ifSDJWTIsEnabled(
              for {
                ...
                result <- (for {
                  ...
                  config <- ZIO.service[AppConfig]
                  _ <- credentialService
                    .generateSDJWTCredential(id, config.credentials.credentialSdJwtExpirationTime)
                    .provideSomeLayer(ZLayer.succeed(walletAccessContext))
                } yield ()).mapError(e => (walletAccessContext, e))
              } yield result
            )
