...
import org.hyperledger.identus.wallet.service.{ManagedDIDService, WalletManagementService}
import zio.*
  ...
  val didStateSyncTrigger = {
    (for {
      ...
      _ <- trigger
        ...
        .repeat(Schedule.spaced(config.credentials.didStateSyncTriggerRecurrenceDelay))
    } yield ()).debug.fork
  }
