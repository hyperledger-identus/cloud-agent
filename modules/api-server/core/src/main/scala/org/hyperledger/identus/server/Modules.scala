...
import org.hyperledger.identus.server.config.{AppConfig, DIDNodeBackend, SecretStorageBackend, ValidatedVaultConfig}
...
object RepoModule {
  ...
  def credentialsDbConfigLayer(appUser: Boolean = true): TaskLayer[DbConfig] = {
    val dbConfigLayer = ZLayer.fromZIO {
      ZIO.service[AppConfig].map(_.credentials.database).map(_.dbConfig(appUser = appUser))
    }
    SystemModule.configLayer >>> dbConfigLayer
  }

  val credentialsContextAwareTransactorLayer: TaskLayer[HikariTransactor[ContextAwareTask]] =
    credentialsDbConfigLayer() >>> TransactorLayer.contextAwareTask

  val credentialsTransactorLayer: TaskLayer[HikariTransactor[Task]] =
    credentialsDbConfigLayer(appUser = false) >>> TransactorLayer.task

  def connectionsDbConfigLayer(appUser: Boolean = true): TaskLayer[DbConfig] = {
    val dbConfigLayer = ZLayer.fromZIO {
      ZIO.service[AppConfig].map(_.connections.database).map(_.dbConfig(appUser = appUser))
    }
    SystemModule.configLayer >>> dbConfigLayer
  }

  val connectionsContextAwareTransactorLayer: TaskLayer[HikariTransactor[ContextAwareTask]] =
    connectionsDbConfigLayer() >>> TransactorLayer.contextAwareTask

  val connectionsTransactorLayer: TaskLayer[HikariTransactor[Task]] =
    connectionsDbConfigLayer(appUser = false) >>> TransactorLayer.task
