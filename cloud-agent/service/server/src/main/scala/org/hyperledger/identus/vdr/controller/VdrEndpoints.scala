...
object VdrEndpoints {
  ...
  val deleteEntry: Endpoint[
    ...
    endpoint.delete
      ...
      .out(jsonBody[DeleteVdrEntryResponse])
      .errorOut(EndpointOutputs.basicFailuresAndNotFound)
      .name("deleteVdrEntry")
  ...
  val operationStatus: PublicEndpoint[
    ...
    endpoint.get
      .in(extractFromRequest[RequestContext](RequestContext.apply))
      .in("vdr" / "operations" / path[String]("operationId"))
      .out(jsonBody[VdrOperationStatusResponse])
      .out(statusCode(StatusCode.Ok).description("VDR operation status"))
      .errorOut(EndpointOutputs.basicFailuresAndNotFound)
      .name("getVdrOperationStatus")
      .summary("Get VDR operation status")
      .tag(tagName)

  val all: List[PublicEndpoint[?, ?, ?, ?]] = List(readEntry, entryProof, operationStatus)
}
